...
- **🤖 AI 爆款分析**：集成 LLM API (Groq/OpenAI)，深度拆解笔记的爆款钩子、情绪价值并提供运营建议。
...
初次使用 AI 分析时，脚本会提示输入以下配置：
- **API Key**: 您的 LLM 服务密钥（OpenAI 或 Groq）。
- **API Host**: 默认为 `https://api.openai.com/v1`。
- **Model**: 例如 `gpt-3.5-turbo` 或 `llama3-8b-8192`。
