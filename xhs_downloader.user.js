...
(function () {
    ...
    const state = {
        ...
        isExtracting: false,
        aiSettings: {
            apiKey: localStorage.getItem('xhs_ai_api_key') || '',
            apiHost: localStorage.getItem('xhs_ai_api_host') || 'https://api.openai.com/v1',
            model: localStorage.getItem('xhs_ai_model') || 'gpt-3.5-turbo'
        }
    };
    ...
    function createUI() {
      ...
      <div id="xhs-dl-menu">
        ...
        <div id="xdl-detail-tools" style="${isExplorePage() ? '' : 'display:none'}">
          ...
          <button class="xdl-btn primary"   id="xdl-ai-analyze" style="background: linear-gradient(135deg, #6a11cb 0%, #2575fc 100%);">🤖 AI 爆款分析</button>
    ...
    `;
        ...
        if (document.getElementById('xdl-ai-analyze')) document.getElementById('xdl-ai-analyze').onclick = analyzeNoteWithAI;
    ...
    async function analyzeNoteWithAI() {
        if (!state.noteData) {
            setStatus('❌ 请先点击“提取笔记内容”后再进行分析');
            return;
        }

        // 检查配置
        if (!state.aiSettings.apiKey) {
            const key = prompt('请输入您的 LLM API Key (OpenAI/Groq格式):');
            if (!key) return;
            state.aiSettings.apiKey = key;
            localStorage.setItem('xhs_ai_api_key', key);

            const host = prompt('请输入 API Host (默认 https://api.openai.com/v1):', state.aiSettings.apiHost);
            if (host) {
                state.aiSettings.apiHost = host;
                localStorage.setItem('xhs_ai_api_host', host);
            }

            const model = prompt('请输入模型名称 (例: gpt-3.5-turbo 或 llama3-8b-8192):', state.aiSettings.model);
            if (model) {
                state.aiSettings.model = model;
                localStorage.setItem('xhs_ai_model', model);
            }
        }

        setStatus('🤖 正在提交 AI 分析，请稍候...');

        const promptText = `
你是一个自媒体运营专家。请分析以下小红书笔记的“爆款基因”：
标题：${state.noteData.title}
正文：${state.noteData.desc}
互动：点赞 ${state.noteData.likes} | 收藏 ${state.noteData.collects}
...
`;
        ...
        try {
            const response = await new Promise((resolve, reject) => {
                GM_xmlhttpRequest({
                    method: 'POST',
                    url: `${state.aiSettings.apiHost}/chat/completions`,
                    headers: {
            ...
            console.error('[XHS-DL] AI 分析失败:', e);
            setStatus(`❌ AI 分析失败: ${e.message}`);
            if (e.message.includes('401')) {
                state.aiSettings.apiKey = '';
                localStorage.removeItem('xhs_ai_api_key');
            }
        }
    }
